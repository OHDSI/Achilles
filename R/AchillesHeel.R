...
#'
#' @details
#' \code{achillesHeel} contains number of rules (authored in SQL) that are executed against achilles
#' results tables.
#'
#' @param connectionDetails              An R object of type \code{connectionDetails} created using the
#'                                       function \code{createConnectionDetails} in the
#'                                       \code{DatabaseConnector} package.
#' @param cdmDatabaseSchema              string name of database schema that contains OMOP CDM. On SQL
#'                                       Server, this should specifiy both the database and the schema,
#'                                       so for example 'cdm_instance.dbo'.
#' @param resultsDatabaseSchema          string name of database schema that we can write final results
#'                                       to. Default is cdmDatabaseSchema. On SQL Server, this should
#'                                       specifiy both the database and the schema, so for example
#'                                       'results.dbo'.
#' @param scratchDatabaseSchema          (OPTIONAL, multi-threaded mode) Name of a fully qualified
#'                                       schema that is accessible to/from the resultsDatabaseSchema,
#'                                       that can store all of the scratch tables. Default is
...
#'                                       Default is cdmDatabaseSchema. On SQL Server, this should
#'                                       specifiy both the database and the schema, so for example
#'                                       'results.dbo'.
#' @param cdmVersion                     Define the OMOP CDM version used: currently supports v5 and
#'                                       above.  Default = "5".
#' @param numThreads                     (OPTIONAL, multi-threaded mode) The number of threads to use
#'                                       to run Achilles in parallel. Default is 1 thread.
#' @param tempHeelPrefix                 (OPTIONAL, multi-threaded mode) The prefix to use for the
#'                                       "temporary" (but actually permanent) Heel tables. Default is
#'                                       "tmpheel"
#' @param dropScratchTables              (OPTIONAL, multi-threaded mode) TRUE = drop the scratch tables
#'                                       (may take time depending on dbms), FALSE = leave them in place
...
#' @param ThresholdMinimalPtMeasDxRx     The minimum percentage of patients with at least 1
#'                                       Measurement, 1 Dx, and 1 Rx
#' @param sqlOnly                        Boolean to determine if Heel should be fully executed. TRUE =
#'                                       just generate SQL files, don't actually run, FALSE = run
#'                                       Achilles Heel
#' @param outputFolder                   Path to store logs and SQL files
#' @param verboseMode                    Boolean to determine if the console will show all execution
#'                                       steps. Default = TRUE
#'
#' @return
...
achillesHeel <- function(connectionDetails,
  ...
  if ((numThreads > 1 || !.supportsTempTables(connectionDetails)) && !sqlOnly) {
    ...
    ParallelLogger::logInfo(sprintf("Dropping scratch Heel tables from schema %s",
                                    scratchDatabaseSchema))

    dropAllScratchTables(connectionDetails = connectionDetails,
                         scratchDatabaseSchema = scratchDatabaseSchema,
                         tempAchillesPrefix = tempHeelPrefix,
                         numThreads = numThreads,
                         tableTypes = c("heel"),
                         outputFolder = outputFolder,
                         verboseMode = verboseMode)

    ParallelLogger::logInfo(sprintf("Temporary Heel tables removed from schema %s",
                                    scratchDatabaseSchema))
  }
  ...
  if (!sqlOnly) {
    if (numThreads == 1) {
      ...
      dummy <- ParallelLogger::clusterApply(cluster = cluster, x = parallelSqls, function(sql) {
        connection <- DatabaseConnector::connect(connectionDetails = connectionDetails)
        DatabaseConnector::executeSql(connection = connection, sql = sql)
  ...
  isDerived <- sapply(parallelFiles, function(parallelFile) {
    grepl(pattern = "derived", parallelFile)
  })

  derivedSqls <- lapply(parallelFiles[isDerived], function(parallelFile) {
    SqlRender::render(sql = "select
             cast(analysis_id as int) as analysis_id, cast(stratum_1 as varchar(255)) as stratum_1, cast(stratum_2 as varchar(255)) as stratum_2, cast(statistic_value as float) as statistic_value, cast(measure_id as varchar(255)) as measure_id
             from @scratchDatabaseSchema@schemaDelim@tempHeelPrefix_@heelName", scratchDatabaseSchema = scratchDatabaseSchema, schemaDelim = ifelse(scratchDatabaseSchema ==
        "#", "s_", "."), tempHeelPrefix = tempHeelPrefix, heelName = gsub(pattern = ".sql", replacement = "", x = basename(parallelFile)))
  })
  ...
  derivedSql <- SqlRender::loadRenderTranslateSql(sqlFilename = "heels/merge_derived.sql",
                                                  ...
                                                  schemaDelim = ifelse(scratchDatabaseSchema == "#",
                                                                       "s_",
                                                                       "."),
                                                  destination = "achilles_rd_0",
                                                  derivedSqls = paste(derivedSqls,
                                                                      collapse = " \nunion all\n "))
  ...
  resultSqls <- lapply(X = parallelFiles[!isDerived], function(parallelFile) {
    SqlRender::render(sql = "select
             ...
             from @scratchDatabaseSchema@schemaDelim@tempHeelPrefix_@heelName", scratchDatabaseSchema = scratchDatabaseSchema, schemaDelim = schemaDelim, tempHeelPrefix = tempHeelPrefix, heelName = gsub(pattern = ".sql", replacement = "", x = basename(parallelFile)))
  })
  ...
  serialFiles <- read.csv(file = system.file("csv",
                                             "heel",
                                             "heel_rules_all.csv",
                                             package = "Achilles"),
                          header = TRUE,
  ...
  for (i in 1:nrow(serialFiles)) {
    ...
    if (i > 1) {
      rdOldId <- as.integer(max(serialFiles$rule_id[serialFiles$destination_table %in% c("results_derived",
                                                                                         "both") & serialFiles$rule_id < newId]))
      hrOldId <- as.integer(max(serialFiles$rule_id[serialFiles$destination_table %in% c("heel_results",
                                                                                         "both") & serialFiles$rule_id < newId]))
    }
    ...
    serialSql <- SqlRender::loadRenderTranslateSql(sqlFilename = sprintf("heels/serial/rule_%d.sql",
                                                   ...
                                                   schema = scratchDatabaseSchema,
                                                   schemaDelim = ifelse(scratchDatabaseSchema ==
        "#", "s_", "."), resultsDatabaseSchema = resultsDatabaseSchema, rdOldId = rdOldId, hrOldId = hrOldId, rdNewId = newId, hrNewId = newId, ThresholdAgeWarning = ThresholdAgeWarning, ThresholdOutpatientVisitPerc = ThresholdOutpatientVisitPerc, ThresholdMinimalPtMeasDxRx = ThresholdMinimalPtMeasDxRx)
  ...
  rdId <- as.integer(max(serialFiles$rule_id[serialFiles$destination_table %in% c("results_derived",
                                                                                  "both")]))
  hrId <- as.integer(max(serialFiles$rule_id[serialFiles$destination_table %in% c("heel_results",
                                                                                  "both")]))
  ...
  if (sqlOnly) {
    ...
    ParallelLogger::logInfo(sprintf("All Achilles SQL scripts can be found in folder: %s",
                                    file.path(outputFolder, "achillesHeel.sql")))
  } else {
    ParallelLogger::logInfo(sprintf("Done. Achilles Heel results can now be found in %s",
                                    resultsDatabaseSchema))
  }
...
.getHeelSql <- function(heelFile,
  ...
  SqlRender::loadRenderTranslateSql(sqlFilename = gsub(pattern = file.path(system.file(package = "Achilles"),
                                                                           "sql/sql_server/"), replacement = "", x = heelFile),
                                    packageName = "Achilles",
                                    dbms = connectionDetails$dbms,
                                    warnOnMissingParameters = FALSE,
                                    cdmDatabaseSchema = cdmDatabaseSchema,
