...
#' @param cdmDatabaseSchema   Fully qualified name of database schema that contains OMOP CDM schema. On
#'                            SQL Server, this should specifiy both the database and the schema, so for
#'                            example, on SQL Server, 'cdm_instance.dbo'.

#' @param resultsDatabaseSchema   Fully qualified name of database schema that we can fetch final
#'                                results from. On SQL Server, this should specifiy both the database
#'                                and the schema, so for example, on SQL Server, 'cdm_results.dbo'.
...
#' @param tempHeelPrefix          (OPTIONAL, multi-threaded mode) The prefix to use for the "temporary"
#'                                (but actually permanent) Heel tables. Default is "tmpheel"
...
launchHeelResultsViewer <- function(connectionDetails,
                                    ...
                                    numThreads = 1,
                                    outputFolder) {
  dependencies <- c("shiny", "DT", "shinydashboard", "magrittr", "tidyr")

  for (d in dependencies) {
    if (!requireNamespace(d, quietly = TRUE)) {
      message <- sprintf("You must install %1s first. You may install it using devtools with the following code:
    \n    install.packages('%2s')
...
#'
#' @description
#' \code{fetchAchillesHeelResults} retrieves the AchillesHeel results for the AChilles analysis to
#' identify potential data quality issues.
#'
#' @details
#' AchillesHeel is a part of the Achilles analysis aimed at identifying potential data quality issues.
#' It will list errors (things that should really be fixed) and warnings (things that should at least
#' be investigated).
#'
...
#'
#' @return
#' An object of type \code{achillesAnalysisResults}
#' @examples
#' \dontrun{
#' connectionDetails <- DatabaseConnector::createConnectionDetails(dbms = "sql server",
#'                                                                 server = "myserver")
...
fetchAchillesAnalysisResults <- function(connectionDetails, resultsDatabaseSchema, analysisId) {
  ...
  result <- list(analysisId = analysisId,
                 analysisName = analysisDetails[1,
                 ]$ANALYSIS_NAME,
