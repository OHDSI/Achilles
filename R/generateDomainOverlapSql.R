...
sqlFile <- "domainOverlap.sql"
  ...
  # creates a matrix of domain overlap possibilities.  If you want to add a domain, you would add to the list directly below.
  domainMatrix <- tidyr::crossing(condition_occurrence = 0:1,
                                  drug_exposure = 0:1, 
                                  device_exposure = 0:1, 
                                  measurement = 0:1,
                                  death = 0:1,
                                  procedure_occurrence = 0:1,
                                  observation_period = 0:1)
  domainMatrixResults <- domainMatrix
  domainMatrixResults <- domainMatrixResults %>% mutate(count = 0,
                                                        proportion = 0,
                                                        dataSource = "")

  
  # Creates notes
  write(x = "-- Analysis 2004: Number of distinct patients that overlap between specific domains", sqlFile, append = TRUE)
  write(x = "-- Bit String Breakdown:   1) Condition Occurrence 2) Drug Exposure 3) Device Exposure 4) Measurement 5) Death 6) Procedure Occurrence 7) Observation Period", sqlFile, append = TRUE)
  ...
  write(x = "select distinct person_id into #obs from @cdmDatabaseSchema.observation_period;", sqlFile, append = TRUE)
  
  write(x = "", sqlFile, append = TRUE)
  
  write(x = "with rawData as (", sqlFile, append = TRUE)
  
  #Begins going through domain matrix by row to calculate overlap of different domain combinations.
  for(i in 1:nrow(domainMatrix)){

    # Builds bit-driven string for strata1
    domainString <- ""
    for(b in 1:ncol(domainMatrix)){domainString <- paste0(domainString,domainMatrixResults[i,b])}
