...
#'                                         exportToJson(connectionDetails, cdmDatabaseSchema="cdm5", resultsDatabaseSchema = "results", 
#'                                                      outputPath=jsonFolderPath)
#'                                         addDataSource(jsonFolderPath, dataSourcesFilePath, dataSourceName)
#'                                      }
#' @export
addDataSource <- function(jsonFolderPath, 
                          dataSourcesFilePath,
                          dataSourceName) {
  
  newEntry <- list(
    name = dataSourceName,
    folder = jsonFolderPath,
    cdmVersion = 5
  )
  ...
  exists <- sapply(j$datasources, function(entry) { 
    tolower(entry$name) == tolower(newEntry$name) & 
      tolower(entry$folder) == tolower(newEntry$folder)
  })
