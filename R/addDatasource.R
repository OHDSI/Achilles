...
#' 
#' @param jsonOutputPath      Path of the Json files generated by \code{exportToJson}.
#' @param dataName  		Name of Achilles report. Default is the base folder of \code{jsonOutputPath}.
#' @param datasourcePath  Path where datasource file will be saved. Default is one folder above the \code{jsonOutputPath}
...
#' 
#' @examples
#' jsonOutputPath <- "your/output/path"
#' connectionDetails <- createConnectionDetails(dbms="sql server", server="yourserver")
#' exportToJson(connectionDetails, cdmDatabaseSchema="cdm5", outputPath=jsonOutputPath)
#' addDatasource(jsonOutputPath, "your_data_name")
#' 
#' @export
addDatasource <- function(jsonOutputPath, dataName = NULL, datasourcePath = NULL, datasourcesFilename = "datasources.json", additionalParam = list(cdmVersion = 5) )
{
  # Parse the folder path.
  folderName <- basename(jsonOutputPath)
  if( is.null(dataName) ){
    dataName <- folderName
  }
  ...
  if( is.null(datasourcePath) ){
    datasourcePath <- file.path( dirname(jsonOutputPath), datasourcesFilename )  
  }
