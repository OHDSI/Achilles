...
#' "PERSON", "PROCEDURE","VISIT", "VISIT_DETAIL"
#'
#' @return
...
exportToJson <- function(connectionDetails,
  ...
  if ("CONDITION" %in% reports) {
    generateConditionTreemap(conn,
                             connectionDetails$dbms,
                             cdmDatabaseSchema,
                             resultsDatabaseSchema,
                             outputPath,
                             vocabDatabaseSchema)
    generateConditionReports(conn,
                             connectionDetails$dbms,
                             cdmDatabaseSchema,
                             resultsDatabaseSchema,
                             outputPath,
  ...
  if ("DRUG_ERA" %in% reports) {
    generateDrugEraTreemap(conn,
                           connectionDetails$dbms,
                           cdmDatabaseSchema,
                           resultsDatabaseSchema,
                           outputPath,
                           vocabDatabaseSchema)
    generateDrugEraReports(conn,
                           connectionDetails$dbms,
                           cdmDatabaseSchema,
                           resultsDatabaseSchema,
                           outputPath,
  ...
  if (("META" %in% reports)) {
    generateMetadataReport(conn,
                           connectionDetails$dbms,
                           cdmDatabaseSchema,
                           resultsDatabaseSchema,
                           outputPath,
                           vocabDatabaseSchema)
    generateCdmSourceReport(conn,
                            connectionDetails$dbms,
                            cdmDatabaseSchema,
                            resultsDatabaseSchema,
                            outputPath,
  ...
  if ("PROCEDURE" %in% reports) {
    generateProcedureTreemap(conn,
                             connectionDetails$dbms,
                             cdmDatabaseSchema,
                             resultsDatabaseSchema,
                             outputPath,
                             vocabDatabaseSchema)
    generateProcedureReports(conn,
                             connectionDetails$dbms,
                             cdmDatabaseSchema,
                             resultsDatabaseSchema,
                             outputPath,
  ...
  if ("VISIT" %in% reports) {
    generateVisitTreemap(conn,
                         connectionDetails$dbms,
                         cdmDatabaseSchema,
                         resultsDatabaseSchema,
                         outputPath,
                         vocabDatabaseSchema)
    generateVisitReports(conn,
                         connectionDetails$dbms,
                         cdmDatabaseSchema,
                         resultsDatabaseSchema,
                         outputPath,
...
generateMetadataReport <- function(conn,
                                   dbms,
                                   cdmDatabaseSchema,
                                   resultsDatabaseSchema,
                                   outputPath,
...
generateDrugEraTreemap <- function(conn,
                                   dbms,
                                   cdmDatabaseSchema,
                                   resultsDatabaseSchema,
                                   outputPath,
  ...
  write(rjson::toJSON(dataDrugEraTreemap, method = "C"),
        paste(outputPath, "/drugera_treemap.json", sep = ""))
  progress <- progress + 1
...
generateConditionTreemap <- function(conn,
  ...
  write(rjson::toJSON(dataConditionTreemap, method = "C"),
        paste(outputPath, "/condition_treemap.json", sep = ""))
  progress <- progress + 1
  utils::setTxtProgressBar(progressBar, progress)
...
generateConditionEraTreemap <- function(conn,
  ...
  write(rjson::toJSON(dataConditionEraTreemap, method = "C"),
        paste(outputPath, "/conditionera_treemap.json", sep = ""))
  progress <- progress + 1
  utils::setTxtProgressBar(progressBar, progress)
...
generateConditionReports <- function(conn,
  ...
  buildConditionReport <- function(concept_id) {
    ...
    report$PREVALENCE_BY_MONTH <- dataPrevalenceByMonth[dataPrevalenceByMonth$CONCEPT_ID == concept_id, c(3,
                                                                                                          4)]
...
generateConditionEraReports <- function(conn,
  ...
  buildConditionEraReport <- function(concept_id) {
    ...
    report$PREVALENCE_BY_MONTH <- dataPrevalenceByMonth[dataPrevalenceByMonth$CONCEPT_ID == concept_id, c(2,
                                                                                                          3)]
    report$LENGTH_OF_ERA <- dataLengthOfEra[dataLengthOfEra$CONCEPT_ID == concept_id, c(2,
                                                                                        3,
                                                                                        4,
                                                                                        5,
                                                                                        6,
...
generateDrugEraReports <- function(conn,
                                   dbms,
                                   cdmDatabaseSchema,
                                   resultsDatabaseSchema,
                                   outputPath,
  ...
  buildDrugEraReport <- function(concept_id) {
    ...
    report$AGE_AT_FIRST_EXPOSURE <- dataAgeAtFirstExposure[dataAgeAtFirstExposure$CONCEPT_ID == concept_id, c(2,
                                                                                                              3,
                                                                                                              4,
                                                                                                              5,
                                                                                                              6,
...
generateProcedureTreemap <- function(conn,
  ...
  write(rjson::toJSON(dataProcedureTreemap, method = "C"),
        paste(outputPath, "/procedure_treemap.json", sep = ""))
  progress <- progress + 1
  utils::setTxtProgressBar(progressBar, progress)
...
generateObservationPeriodReport <- function(conn,
  ...
  # 1.  Title: Age at time of first observation a.  Visualization: Histogram b.  Category: Age c.Value:
  # # of persons
  ...
  # 3.  Title: Length of observation a.Visualization: bar b.Category: length of observation period, 30d
  # increments c.Values: # of persons
  ...
  # 5.  Title: Observation period length distribution, by gender a.Visualization: side-by-side boxplot
  # b.Category: Gender c.Values: Min/25%/Median/95%/Max length of observation period
...
generateDataDensityReport <- function(conn,
  ...
  # Condition/Drug/Procedure/Observation c.Values: Min/25%/Median/95%/Max number of distinct concepts
  # per person
...
generateMeasurementTreemap <- function(conn,
  ...
  write(rjson::toJSON(dataMeasurementTreemap, method = "C"),
        paste(outputPath, "/measurement_treemap.json", sep = ""))
  progress <- progress + 1
  utils::setTxtProgressBar(progressBar, progress)
...
generateMeasurementReports <- function(conn,
  ...
  buildMeasurementReport <- function(concept_id) {
    ...
    report$RECORDS_BY_UNIT <- dataRecordsByUnit[dataRecordsByUnit$MEASUREMENT_CONCEPT_ID == concept_id, c(4,
                                                                                                          5)]
...
generateObservationTreemap <- function(conn,
  ...
  write(rjson::toJSON(dataObservationTreemap, method = "C"),
        paste(outputPath, "/observation_treemap.json", sep = ""))
  progress <- progress + 1
  utils::setTxtProgressBar(progressBar, progress)
...
generateDeathReports <- function(conn,
  ...
  # lineplot b.Trellis category: age decile c.X-axis: year d.y-axis: condition prevalence (% persons)
  ...
  # 3.  Title: Death records by type a.Visualization: pie b.Category: death type c.value: % of records
