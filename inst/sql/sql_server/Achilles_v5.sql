...
(
  ...
  FROM
  (
    select MIN(YEAR(observation_period_start_date)) - P.YEAR_OF_BIRTH as age_value
...
JOIN dbo.PERSON p on op.person_id = p.person_id
where op.rn = 1
;

with overallStats (age_decile, avg_value, stdev_value, min_value, max_value, total) as
(
  select age_decile,
    avg(1.0 * count_value) as avg_value,
    stdev(count_value) as stdev_value,
...
(
  ...
  from @cdm_database_schema.drug_exposure de1
  	inner join @cdm_database_schema.drug_cost dc1
...
(
  ...
  from @cdm_database_schema.drug_exposure de1
  inner join @cdm_database_schema.drug_cost dc1 on de1.drug_exposure_id = dc1.drug_exposure_id
...
(
  ...
  from drug_exposure de1 inner join @cdm_database_schema.drug_cost dc1 on de1.drug_exposure_id = dc1.drug_exposure_id
...
with rawData(stratum1_id, count_value) as
(
  select procedure_concept_id as stratum1_id,
    paid_copay as count_value
  from @cdm_database_schema.procedure_occurrence po1 
  inner join @cdm_database_schema.procedure_cost pc1 on po1.procedure_occurrence_id = pc1.procedure_occurrence_id
  where paid_copay is not null
),
overallStats (stratum1_id, avg_value, stdev_value, min_value, max_value, total) as
(
