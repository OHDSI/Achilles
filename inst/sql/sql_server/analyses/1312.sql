...
SELECT 
	...
	FLOOR((YEAR(visit_start_date) - p.year_of_birth) / 10) AS stratum_3,
	COUNT_BIG(DISTINCT p1.person_id) AS count_value
FROM 
	@cdmDatabaseSchema.person p
JOIN 
	@cdmDatabaseSchema.visit_detail vd ON p.person_id = vd.person_id
GROUP BY 
	YEAR(vd.visit_start_date),
	p.gender_concept_id,
	FLOOR((YEAR(visit_start_date) - p.year_of_birth) / 10)
)
SELECT
	1312 as analysis_id,
