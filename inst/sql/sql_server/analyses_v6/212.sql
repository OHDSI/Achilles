...
select 212 as analysis_id,   
	CAST(YEAR(visit_start_datetime) AS VARCHAR(255)) as stratum_1,
	CAST(p1.gender_concept_id AS VARCHAR(255)) as stratum_2,
	CAST(floor((year(visit_start_datetime) - p1.year_of_birth)/10) AS VARCHAR(255)) as stratum_3,
...
group by 
	YEAR(visit_start_datetime),
	p1.gender_concept_id,
	floor((year(visit_start_datetime) - p1.year_of_birth)/10)
;
