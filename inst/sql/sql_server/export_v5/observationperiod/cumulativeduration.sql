...
from (select * from @cdm_database_schema.ACHILLES_results where analysis_id = 108) ar1
inner join
(
	select * from @cdm_database_schema.ACHILLES_results where analysis_id = 108
) ar2 on ar1.analysis_id = ar2.analysis_id and cast(ar1.stratum_1 as int) <= cast(ar2.stratum_1 as int),
(
	select count_value from @cdm_database_schema.ACHILLES_results where analysis_id = 1
) denom
